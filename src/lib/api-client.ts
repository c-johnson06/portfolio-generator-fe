...
export class ApiClient {
  ...
  static async post<T = unknown>(endpoint: string, data: Record<string, unknown>): Promise<T> {
  ...
  static async put<T = unknown>(endpoint: string, data: Record<string, unknown>): Promise<T> {
    const response = await this.fetch(endpoint, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
